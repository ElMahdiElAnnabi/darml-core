// Darml inference wrapper — ESP32 via tflm_esp32 + eloquent_tinyml.
//
// EloquentTinyML 3.x splits the runtime ("tflm_esp32") from the high-level
// wrapper ("eloquent_tinyml"). The runtime header MUST be included before
// the wrapper, and the wrapper's `Eloquent::TF::Sequential` template takes
// the op-resolver size and tensor-arena size as compile-time parameters.
//
// Placeholders below ({{INPUT_SIZE}}, etc.) are substituted at template-
// render time by the Darml builder before this file ever reaches the
// compiler.
...
// Sized statically per the resolver template, so unused slots cost nothing
// at runtime; flash impact is a few hundred bytes of registration table.
...
// Register every op MicroMutableOpResolver supports. Approximates the
// AllOpsResolver behavior — any model whose ops compile into the linked
// kernels will load. Op kernels themselves are pulled in by the linker on
// demand, so registering an unused op only costs a registration-table slot.
void register_ops() {
